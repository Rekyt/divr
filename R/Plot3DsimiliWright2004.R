...
LESPlot3D<-function(yourdata,Xaxis,Yaxis,Zaxis,xlab="X",ylab="Y",zlab="Z",logxyz="",Rotate=110,Flip=25){
  ...
  if(grepl("x",logxyz)==T){
    ...
    Xrange<-Xrange[which(Xrange>range(yourdata[[Xaxis]],na.rm = T)[1] & Xrange<range(yourdata[[Xaxis]],na.rm = T)[2])]
  }else{
  ...
  if(grepl("y",logxyz)==T){
    ...
    Yrange<-Yrange[which(Yrange>range(yourdata[[Yaxis]],na.rm = T)[1] & Yrange<range(yourdata[[Yaxis]],na.rm = T)[2])]
  }else{
  ...
  if(grepl("z",logxyz)==T){
    ...
    Zrange<-Zrange[which(Zrange>range(yourdata[[Zaxis]],na.rm = T)[1] & Zrange<range(yourdata[[Zaxis]],na.rm = T)[2])]
  }else{
...
#plan XY  
  ...
  coord1 <- trans3D(Xrange,rep(Ymin,length(Xrange)),z = rep(Zmin, length(Xrange)), pmat = pmat)
  coord2 <- trans3D(Xrange,rep(Ymax,length(Xrange)),z = rep(Zmin, length(Xrange)), pmat = pmat)
  segments2D(x0 = coord1$x,y0 =  coord1$y,x1 = coord2$x,y1 =  coord2$y,lwd=1.5,add=T,col="grey", colkey = FALSE)
  coord1 <- trans3D(rep(Xmin,length(Yrange)),Yrange,z = rep(Zmin, length(Yrange)), pmat = pmat)
  coord2 <- trans3D(rep(Xmax,length(Yrange)),Yrange,z = rep(Zmin, length(Yrange)), pmat = pmat)
  segments2D(x0 = coord1$x,y0 =  coord1$y,x1 = coord2$x,y1 =  coord2$y,lwd=1.5,add=T,col="grey", colkey = FALSE)
  
  CoordLab <- trans3D(x = rep(Xmax+(10/100)*(max(yourdata[[Xaxis]],na.rm = T)-min(yourdata[[Xaxis]],na.rm = T)),length(Yrange)),
                      y = Yrange,
